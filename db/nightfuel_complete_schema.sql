-- ╔═══════════════════════════════════════════════════════════════════════════╗
-- ║          NightFuel — COMPLETE DATABASE SCHEMA (All Services)            ║
-- ║          Version: 2.0  |  Date: 2026-02-26                             ║
-- ╠═══════════════════════════════════════════════════════════════════════════╣
-- ║  This file is the canonical, single-source database definition for the  ║
-- ║  entire NightFuel platform. It covers all 13 microservice databases.    ║
-- ║                                                                         ║
-- ║  HOW TO RUN                                                             ║
-- ║  ─────────────────────────────────────────────────────────────────────  ║
-- ║  Local (Docker):                                                        ║
-- ║    psql -U postgres -f nightfuel_complete_schema.sql                    ║
-- ║                                                                         ║
-- ║  Supabase (one project per service):                                    ║
-- ║    Copy each service section individually into the SQL editor.          ║
-- ║    Each section is self-contained between its \connect markers.         ║
-- ║                                                                         ║
-- ║  NOTE: The \connect metacommand requires psql. If using a GUI tool      ║
-- ║  (pgAdmin, TablePlus, Supabase UI), run each section separately.        ║
-- ╠═══════════════════════════════════════════════════════════════════════════╣
-- ║  DATABASES                    PORT   SERVICE                            ║
-- ║  ─────────────────────────── ────── ──────────────────────────────────  ║
-- ║  nightfuel_auth               3001   auth-service                       ║
-- ║  nightfuel_users              3009   user-service                       ║
-- ║  nightfuel_shifts             3002   shift-service                      ║
-- ║  nightfuel_plans              3005   plan-service                       ║
-- ║  nightfuel_meals              3006   meal-service                       ║
-- ║  nightfuel_workouts           3011   exercise-service                   ║
-- ║  nightfuel_sleep              3010   sleep-service                      ║
-- ║  nightfuel_progress           3007   progress-service                   ║
-- ║  nightfuel_notifications      3008   notification-service               ║
-- ║  nightfuel_billing            3012   subscription-service               ║
-- ║  nightfuel_state              ----   state-service (internal)           ║
-- ║  nightfuel_community          3013   community-service                  ║
-- ║  nightfuel_chat               3014   chat-service                       ║
-- ╚═══════════════════════════════════════════════════════════════════════════╝


-- ┌─────────────────────────────────────────────────────────────────────────┐
-- │  STEP 1 — Create all databases (run as superuser on postgres database)  │
-- └─────────────────────────────────────────────────────────────────────────┘
...
-- ═══════════════════════════════════════════════════════════════════════════════
--  §1  AUTH SERVICE  ·  nightfuel_auth
--      Handles: user identity, password hashes, JWT refresh tokens
-- ═══════════════════════════════════════════════════════════════════════════════

\connect nightfuel_auth
...
-- ═══════════════════════════════════════════════════════════════════════════════
--  §2  USER SERVICE  ·  nightfuel_users
--      Handles: extended profiles, preferences, onboarding, coach management
-- ═══════════════════════════════════════════════════════════════════════════════

\connect nightfuel_users
...
-- Extended profile data (cross-service ref: auth_db.users.id via user_id)
...
-- Fitness goals, dietary preferences, and workout configuration
CREATE TABLE IF NOT EXISTS user_preferences (
  ...
  sleep_window_start          VARCHAR(5),      -- HH:MM (e.g. 22:00)
  sleep_window_end            VARCHAR(5),      -- HH:MM (e.g. 06:00)
...
-- Specialist/trainer profile extensions
...
-- Relationships: which coach manages which clients
...
-- Real-time biological and performance snapshot for AI consumption
CREATE TABLE IF NOT EXISTS user_statuses (
  id                  UUID             PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id             UUID             NOT NULL UNIQUE,
  fatigue_score       DOUBLE PRECISION NOT NULL DEFAULT 0,
  circadian_peak_time VARCHAR(5),      -- HH:MM
  circadian_low_time  VARCHAR(5),      -- HH:MM
  adherence_rate      DOUBLE PRECISION NOT NULL DEFAULT 0,
  current_streak      INTEGER          NOT NULL DEFAULT 0,
  current_tdee        DOUBLE PRECISION,
  weight_trend        VARCHAR(10),     -- UP | DOWN | STABLE
...
-- ═══════════════════════════════════════════════════════════════════════════════
--  §3  SHIFT SERVICE  ·  nightfuel_shifts
--      Handles: work shift entries, sleep windows, rotation patterns
-- ═══════════════════════════════════════════════════════════════════════════════

\connect nightfuel_shifts
...
-- Individual work shift entries with associated sleep windows
...
-- Multi-day shift rotation cycle definitions (e.g. 4-on-4-off)
CREATE TABLE IF NOT EXISTS rotation_patterns (
  id           UUID         PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id      UUID         NOT NULL,
  pattern_name VARCHAR(100) NOT NULL DEFAULT 'Default',
  cycle_days   INTEGER      NOT NULL,
  pattern      JSONB        NOT NULL,  -- Array of { shift_type, start_time, end_time, is_day_off }
...
-- ═══════════════════════════════════════════════════════════════════════════════
--  §4  PLAN SERVICE  ·  nightfuel_plans
--      Handles: AI-generated daily nutrition+workout plans, protocol templates
-- ═══════════════════════════════════════════════════════════════════════════════

\connect nightfuel_plans
...
-- Coach/specialist-created reusable plan templates
...
-- AI-generated daily nutrition + workout plans with versioning support
CREATE TABLE IF NOT EXISTS day_plans (
  id                   UUID         PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id              UUID         NOT NULL,
  plan_date            DATE         NOT NULL,
  shift_id             UUID,                             -- cross-service ref → nightfuel_shifts.shifts.id
  circadian_profile_id UUID,                             -- cross-service ref → circadian-engine output
  protocol_id          UUID,
  plan_version         INTEGER      NOT NULL DEFAULT 1,
  plan                 JSONB        NOT NULL,            -- full structured plan payload
...
-- ═══════════════════════════════════════════════════════════════════════════════
--  §5  MEAL SERVICE  ·  nightfuel_meals
--      Handles: food catalog, meal logging, macros, recipes, fasting
-- ═══════════════════════════════════════════════════════════════════════════════

\connect nightfuel_meals
...
-- Master food catalog with nutrition data and regional/dietary flags
...
-- User-logged meals with macro totals and shift-adherence tracking
CREATE TABLE IF NOT EXISTS meal_logs (
  id             UUID             PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id        UUID             NOT NULL,
  logged_at      TIMESTAMPTZ      NOT NULL DEFAULT NOW(),
  meal_type      "MealType"       NOT NULL,
  food_items     JSONB            NOT NULL,   -- Array of { food_id, name, quantity_g, calories, protein, carbs, fat }
...
-- Meal planner weekly layouts
CREATE TABLE IF NOT EXISTS meal_plans (
  id          UUID        PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id     UUID        NOT NULL,
  week_start  DATE        NOT NULL,
  plan_data   JSONB       NOT NULL,   -- { "YYYY-MM-DD": { BREAKFAST: [...], LUNCH: [...] } }
  created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
-- System catalog of healthy meals and recipes
...
-- User-generated grocery lists linked to a meal plan
CREATE TABLE IF NOT EXISTS grocery_lists (
  id          UUID        PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id     UUID        NOT NULL,
  plan_id     UUID,                   -- cross-service ref → nightfuel_plans.day_plans.id
  week_start  DATE        NOT NULL,
  items       JSONB       NOT NULL DEFAULT '[]',   -- Array of { name, qty, unit, checked }
  created_at  TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at  TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS fasting_logs (
  ...
  protocol_id  VARCHAR(20),    -- e.g. '16:8', '18:6', '5:2', 'OMAD'
  notes        TEXT,
  created_at   TIMESTAMPTZ     NOT NULL DEFAULT NOW()
);
...
-- ═══════════════════════════════════════════════════════════════════════════════
--  §6  EXERCISE SERVICE  ·  nightfuel_workouts
--      Handles: exercise library, workout logs, routines, 1RM history
-- ═══════════════════════════════════════════════════════════════════════════════

\connect nightfuel_workouts
...
-- Pre-seeded exercise catalog (sourced from ExerciseDB API)
CREATE TABLE IF NOT EXISTS library_exercises (
  id           UUID          PRIMARY KEY DEFAULT uuid_generate_v4(),
  external_id  VARCHAR(50)   UNIQUE,          -- ExerciseDB ID
...
CREATE TABLE IF NOT EXISTS workout_sets (
  ...
  library_ex_id   UUID,                         -- optional ref to library_exercises
...
-- User-created and coach-created workout templates
CREATE TABLE IF NOT EXISTS workout_routines (
  id          UUID        PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id     UUID,                    -- NULL for system/global routines
  title       VARCHAR(255) NOT NULL,
  description TEXT,
  split_type  "SplitType",
...
-- Historical 1-rep max logs per user per exercise
CREATE TABLE IF NOT EXISTS one_rm_logs (
  ...
  method           VARCHAR(20)      NOT NULL DEFAULT 'EPLEY',  -- EPLEY | BRZYCKI | ACTUAL
  date             DATE             NOT NULL,
  created_at       TIMESTAMPTZ      NOT NULL DEFAULT NOW()
);
...
-- ═══════════════════════════════════════════════════════════════════════════════
--  §7  SLEEP SERVICE  ·  nightfuel_sleep
--      Handles: sleep session logs, quality scoring, wearable sync, preferences
-- ═══════════════════════════════════════════════════════════════════════════════

\connect nightfuel_sleep
...
-- Individual sleep entries with circadian alignment scoring
...
-- Per-user sleep goals and wind-down configuration
CREATE TABLE IF NOT EXISTS sleep_preferences (
  id                 UUID             PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id            UUID             NOT NULL UNIQUE,
  target_duration    INTEGER          NOT NULL DEFAULT 480,   -- minutes
  wind_down_duration INTEGER          NOT NULL DEFAULT 30,    -- minutes
  wake_time_target   VARCHAR(5),                              -- HH:MM
  sleep_time_target  VARCHAR(5),                              -- HH:MM
  temperature_target DOUBLE PRECISION,                        -- Celsius
  noise_preference   VARCHAR(20)      DEFAULT 'SILENT',       -- SILENT | WHITE_NOISE | NATURE
  updated_at         TIMESTAMPTZ      NOT NULL DEFAULT NOW()
);

-- Wearable device connection records
...
-- ═══════════════════════════════════════════════════════════════════════════════
--  §8  PROGRESS SERVICE  ·  nightfuel_progress
--      Handles: daily tracking aggregates, body metrics, streaks, AI reports
-- ═══════════════════════════════════════════════════════════════════════════════

\connect nightfuel_progress
...
-- Daily aggregated tracking: calories, macros, hydration, adherence
...
-- Adherence streak tracking per user
...
-- Weight, body fat, and muscle mass time-series history
...
-- Progress photos with body part tagging
...
-- LLM token usage auditing per user (cost accountability)
...
-- ═══════════════════════════════════════════════════════════════════════════════
--  §9  NOTIFICATION SERVICE  ·  nightfuel_notifications
--      Handles: in-app inbox, push subscriptions (VAPID + Expo), preferences
-- ═══════════════════════════════════════════════════════════════════════════════

\connect nightfuel_notifications
...
-- Per-user notification toggle settings and quiet hours configuration
CREATE TABLE IF NOT EXISTS notification_preferences (
  ...
  quiet_hours_start        VARCHAR(5)  NOT NULL DEFAULT '22:00',  -- HH:MM
  quiet_hours_end          VARCHAR(5)  NOT NULL DEFAULT '07:00',  -- HH:MM
  created_at               TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at               TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

-- User notification inbox (in-app messages)
CREATE TABLE IF NOT EXISTS notifications (
  ...
  title      VARCHAR(255)       NOT NULL,
  body       TEXT               NOT NULL,
  data       JSONB,             -- extra payload e.g. { plan_id, shift_id, report_id }
  action_url TEXT,              -- deep-link target
  is_read    BOOLEAN            NOT NULL DEFAULT FALSE,
  created_at TIMESTAMPTZ        NOT NULL DEFAULT NOW()
);

-- Web Push (VAPID) and Expo push token registrations per user-device
CREATE TABLE IF NOT EXISTS push_subscriptions (
  id          UUID           PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id     UUID           NOT NULL,
  endpoint    TEXT           NOT NULL UNIQUE,    -- browser push URL or Expo token
  p256dh      TEXT,                              -- Web Push encryption public key
...
-- ═══════════════════════════════════════════════════════════════════════════════
--  §10 SUBSCRIPTION SERVICE  ·  nightfuel_billing
--      Handles: subscription tiers, Stripe IDs, billing events audit log
-- ═══════════════════════════════════════════════════════════════════════════════

\connect nightfuel_billing
...
-- One subscription record per user (FREE tier created on registration)
...
CREATE TABLE IF NOT EXISTS subscription_events (
  id         UUID        PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id    UUID        NOT NULL,
  event_type VARCHAR(50) NOT NULL,   -- CREATED | UPGRADED | DOWNGRADED | CANCELLED | PAYMENT_FAILED | RENEWED
  from_tier  VARCHAR(20),
  to_tier    VARCHAR(20),
  metadata   JSONB,                  -- Stripe session/invoice data
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
-- ═══════════════════════════════════════════════════════════════════════════════
--  §11 STATE SERVICE  ·  nightfuel_state
--      Handles: aggregated real-time user state consumed by the AI pipeline
-- ═══════════════════════════════════════════════════════════════════════════════

\connect nightfuel_state
...
-- Aggregated real-time state snapshot used by ai-pipeline plan generation
CREATE TABLE IF NOT EXISTS user_states (
  id                       UUID             PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id                  UUID             NOT NULL UNIQUE,
  last_7_days_adherence    DOUBLE PRECISION NOT NULL DEFAULT 1.0,   -- 0.0 – 1.0
  avg_sleep_quality        DOUBLE PRECISION NOT NULL DEFAULT 7.0,   -- 1 – 10
  fatigue_level            DOUBLE PRECISION NOT NULL DEFAULT 3.0,   -- 1 – 10
  current_weight_kg        DOUBLE PRECISION,
  target_weight_kg         DOUBLE PRECISION,
  current_calorie_target   DOUBLE PRECISION,
  current_protein_target_g DOUBLE PRECISION,
  training_phase           VARCHAR(30)      NOT NULL DEFAULT 'HYPERTROPHY',  -- HYPERTROPHY | STRENGTH | CUT | MAINTENANCE
  cycle_week               INTEGER          NOT NULL DEFAULT 1,
  last_event_id            UUID,            -- ID of the last Redis event that triggered an update
  last_processed_at        TIMESTAMPTZ      NOT NULL DEFAULT NOW(),
  updated_at               TIMESTAMPTZ      NOT NULL DEFAULT NOW()
);

-- Feature flag overrides per user (for A/B testing and early access)
...
-- ═══════════════════════════════════════════════════════════════════════════════
--  §12 COMMUNITY SERVICE  ·  nightfuel_community
--      Handles: social posts, comments, challenges, leaderboards, reactions
-- ═══════════════════════════════════════════════════════════════════════════════

\connect nightfuel_community
...
-- Tracks which users are participating in which challenges
...
-- User-generated content moderation reports
...
-- ═══════════════════════════════════════════════════════════════════════════════
--  §13 CHAT SERVICE  ·  nightfuel_chat
--      Handles: coach ↔ client real-time messaging, conversation management
-- ═══════════════════════════════════════════════════════════════════════════════

\connect nightfuel_chat
...
CREATE TABLE IF NOT EXISTS conversations (
  id            UUID        PRIMARY KEY DEFAULT uuid_generate_v4(),
  participant_a UUID        NOT NULL,   -- typically coach
  participant_b UUID        NOT NULL,   -- typically client
  last_message  TEXT,
...
CREATE TABLE IF NOT EXISTS messages (
  ...
  text            TEXT,
  attachment_url  TEXT,
  metadata        JSONB,        -- for PLAN_SHARE: { plan_id, plan_date }
  is_read         BOOLEAN       NOT NULL DEFAULT FALSE,
  created_at      TIMESTAMPTZ   NOT NULL DEFAULT NOW(),
...
-- AI assistant conversation threads (separate from coach chat)
...
-- Messages in an AI assistant thread
...
CREATE INDEX IF NOT EXISTS idx_coach_profiles_active     ON coach_profiles(active) WHERE active = TRUE;
...
-- ║  Triggers:   22                                                         ║
-- ╠═══════════════════════════════════════════════════════════════════════════╣
-- ║  TABLE MAP (database → tables)                                          ║
-- ║                                                                         ║
-- ║  nightfuel_auth         users, refresh_tokens                           ║
-- ║  nightfuel_users        user_profiles, user_preferences,                ║
-- ║                         coach_profiles, coach_client_relations,         ║
-- ║                         user_statuses                                   ║
-- ║  nightfuel_shifts       shifts, rotation_patterns                       ║
-- ║  nightfuel_plans        protocol_templates, day_plans                   ║
-- ║  nightfuel_meals        food_items, meal_logs, meal_plans,              ║
-- ║                         recipes, grocery_lists, fasting_logs            ║
-- ║  nightfuel_workouts     library_exercises, workouts, workout_sets,      ║
-- ║                         workout_routines, one_rm_logs                   ║
-- ║  nightfuel_sleep        sleep_sessions, sleep_preferences,              ║
-- ║                         wearable_connections                            ║
-- ║  nightfuel_progress     daily_progress, streaks, body_metrics,          ║
-- ║                         progress_photos, performance_reports,           ║
-- ║                         ai_usage_logs                                   ║
-- ║  nightfuel_notifications notification_preferences, notifications,       ║
-- ║                           push_subscriptions                            ║
-- ║  nightfuel_billing      subscriptions, subscription_events              ║
-- ║  nightfuel_state        user_states, feature_flags                      ║
-- ║  nightfuel_community    posts, post_likes, comments, challenges,        ║
-- ║                         challenge_participants, content_reports         ║
-- ║  nightfuel_chat         coach_profiles, conversations, messages,        ║
